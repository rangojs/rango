...
You have JSON endpoints built with `path.json()` and first-party TypeScript code that calls them — and you'd rather call them like functions than hand-write `fetch` + URL building at every call site. [Response routes](/docs/rango/features/response-routes) already ship typed responses — `RouteResponse<typeof patterns, "name">` resolves to the bare payload, inferred from your handler with no codegen — so all that's missing is a small client that wraps that inference.

This is a recipe, not a framework feature: copy the helper below into your app. It depends only on type-only imports from `@rangojs/router` (`RouteResponse`, `ExtractParams`, `ProblemDetails`, `UrlPatterns`), which are erased at build time, so it runs anywhere a `fetch` does — browser, worker, or server. Nothing new to install or version. One scope note: the typed client is a first-party TypeScript convenience. External and third-party consumers in any language use the plain wire directly — bare JSON on success, RFC 9457 `application/problem+json` on error — which needs no client at all.
...
- **Output typed** from the handler's return (`RouteResponse`) — the bare payload, not `any`, no `.data` wrapper.
...
`search` and `body` are not route-typed; see [Pitfalls and variations](#pitfalls-and-variations).

## The two inputs

1. **The `urls()` patterns type** — the type source, passed as the first type argument. `typeof apiShopPatterns` carries the per-route response payloads and patterns. Only the type is needed, so import the module with `import type`.
...
- **Client-safe by construction.** The helper imports only types from `@rangojs/router` and builds URLs itself: `fillPath` substitutes standard `:param`, optional `:param?`, and constrained `:param(a|b)` forms into the pattern. It does **not** use `createReverse` — that's a server/RSC-only export that throws in the browser — so `createApiClient` works in client components, workers, and on the server alike. For exotic patterns or strict trailing-slash policies, swap in your own builder (or `ctx.reverse()` / `router.reverse()` on the server).
...
- **Params are route-typed; search and body are not.** Path params come from the pattern via `ExtractParams`, so they're precise and required. `search` is generically typed (`Record<string, string | number | boolean>`) and `body` is `unknown` (serialized to JSON). Typed request _input_ needs a declared schema layer, which is intentionally out of scope here — thread per-route schemas in yourself if you want typed search/body.
- **Verb-agnostic wire.** Rango response routes do not dispatch on HTTP method — `.get`/`.post`/etc. set the request method but hit the same handler (branch on `ctx.request.method` inside it if you need to). Use whichever verb reads best for the operation. Response-route `cache()` only serves and stores GET and HEAD.
- **Prefer returns over throws?** Branch on `res.ok` yourself: the wire is the bare value on 2xx and `ProblemDetails` on non-2xx. Wrapping calls in a `{ ok, data } | { ok: false, error }` result type is a small variation on the same helper.
- **Third parties.** The typed client is TypeScript-only and needs your route types. External consumers use the plain wire as-is; no client required.

See [Response Routes](/docs/rango/features/response-routes) for the endpoint side, and [Type Safety](/docs/rango/concepts/typesafety) for how `RouteResponse` / `PathResponse` inference works.
