...
All Storefront API data flows through a shared client built with [`@shopify/storefront-api-client`](https://www.npmjs.com/package/@shopify/storefront-api-client), exported as `storefront` from `lib/shopify/storefront.ts`. Queries are tagged `#graphql` and validated against the live schema by [`@shopify/api-codegen-preset`](https://www.npmjs.com/package/@shopify/api-codegen-preset) (see [Codegen](#codegen)).

For Shopify admin setup and required token scopes, see [Storefront API Permissions](/docs/shop-template/reference/storefront-api-permissions).

> The Customer Account API is a separate endpoint and schema. It still uses the hand-rolled `customerAccountFetch` in `lib/shopify/customer-account.ts` and is not covered by the Storefront client or codegen — the first-party client can't talk to it.
...
Always use Shopify AI Toolkit to search current Shopify documentation and validate the final operation. Then use [`/vercel-shop:shopify-graphql-reference`](/docs/shop-template/skills/shopify-graphql-reference) for Vercel Shop integration conventions. The Vercel Shop skill is not a schema source.
...
1. **`"use cache"`** - caches the operation result, keyed on its arguments, against the `default` cache profile (`cacheProfiles` in `src/router.tsx`)
2. **`cacheLife("max")`** - a no-op in this codebase (`@/lib/cache`); the cache lifetime is set by the profile, not per-call. It is kept to preserve the data layer's original very-long-lived, tag-invalidated intent
3. **`cacheTag(...)`** - assigns tags for granular invalidation via `revalidateTag()` / `updateTag()`
...
Read operations choose cache behavior from their render role. Each `"use cache"` directive resolves against a named profile in `cacheProfiles` (`createRouter`, `src/router.tsx`), and that profile sets the cache lifetime (TTL/SWR):

| Render role                                                                     | Treatment                                                        |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| Public identity/body (single product, collection, or menu)                      | Plain `"use cache"` → the `default` profile                      |
| Public results resolved after request inputs such as filters, search, or cursor | `"use cache: remote"` → the `remote` profile                     |
| Cart, session, authorization, or Customer Account data                          | Request-scoped and uncached; never a public cache profile        |
| Mutation                                                                        | No read-cache directive; invalidate affected state after success |

Cache lifetime comes from the chosen profile, not from `cacheLife()` (a no-op here). The backing store is `VercelCacheStore` (via `@vercel/functions`) in production and an in-memory store in development, selected by `env.VERCEL` in the router's `cache` option. Combined with webhook-driven invalidation, the tags below purge the matching cache entries when Shopify changes. Tags follow a hierarchy:
...
Invalidate with `revalidateTag("product-blue-tee")` (a variadic hard purge) or, inside a server action, `updateTag("cart")` — both re-exported from `@/lib/cache`. Cart mutations must always call `invalidateCartCache()` from `lib/cart/server` - this is a hard requirement.
...
`@shopify/storefront-api-client` never throws on its own — `request()` resolves to `{ data?, errors? }`. The template applies its contract through `assertStorefrontOk(response, operation)` in `lib/shopify/errors.ts`:

**Transport / GraphQL failure** - if `errors` is present and there's no `data`, it throws with the operation name and error detail.
...
| `lib/cache.ts`                          | `cacheTag` / `revalidateTag` / `updateTag` re-exports; no-op `cacheLife` |
