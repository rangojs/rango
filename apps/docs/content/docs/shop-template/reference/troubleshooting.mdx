...
The most common cause is missing Storefront API permissions. In your Shopify admin, go to **Settings → Apps and sales channels → Headless → your storefront** and compare your token scopes against [Storefront API Permissions](/docs/shop-template/reference/storefront-api-permissions).
...
Shopify serves images from its CDN, and the app `Image` component (`@/components/image`) points `src` straight at `cdn.shopify.com` — there is no build-time optimizer or domain allowlist to configure. If images don't load, confirm the `src` resolves to a `cdn.shopify.com` URL (the component only appends `?width=` transforms for Shopify hosts; non-Shopify sources render as a plain `<img>`), and check the browser network tab for a failing CDN request or a missing `alt`/dimensions warning.
...
Re-check and validate the operation with Shopify AI Toolkit first. Then use the [`shopify-graphql-reference` skill](/docs/shop-template/skills/shopify-graphql-reference) for the template integration and cache role.
...
To get near-instant updates, set up Shopify webhooks pointed at `/api/webhooks/shopify`. See [Shopify Integration → Webhooks](/docs/shop-template/shopify#webhooks-and-cache-invalidation) for the full setup guide.

## Locale or currency not changing

Multi-locale support requires Shopify Markets to be enabled. The template ships as single-locale by default. Run [`/vercel-shop:enable-shopify-markets`](/docs/shop-template/skills/enable-shopify-markets) to add multi-locale and multi-currency routing.
