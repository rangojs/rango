...
After [creating your project](/docs/shop-template/getting-started), `create-vercel-shop` installs the project plugins:

- `vercel-shop` for initialization and storefront skills plus drift and upgrade-planning commands
- `vercel-plugin` for generic Vercel deployment guidance
...
| ------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rango-agent-rules` | Rango       | Reminds agents this storefront runs on @rangojs/router (not Next.js) and to read the bundled Rango skills before writing code.                                                                       |
...
To register `/rango` as a project skill instead of relying only on the managed
rule, run:
...
Inspect `.agents/skills/rango` and `.claude/skills/rango` first and stop if
either exists. The external CLI may auto-confirm inside an agent session, so
omitting `--yes` is not collision protection.
