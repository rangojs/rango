...
This page covers the Shopify admin configuration that the PDP expects. For how the page renders and how variant selection works, see [Anatomy → PDP](/docs/shop-template/anatomy/pages/pdp).
...
```ts
export const productMetafieldIdentifiers: Array<{
  key: string;
  namespace: string;
}> = [
