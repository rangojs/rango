...
export async function getPrivacyPolicy(locale: string = defaultLocale) {
  "use cache";
  cacheLife("max");
  cacheTag("policies", "privacy-policy");

  const response = await storefront.request<ShopifyPrivacyPolicyResponse>(
...
- **`cacheLife("max")`** is a compatibility no-op (`@/lib/cache`) — the actual lifetime comes from the cache profile the `"use cache"` directive resolves to in `cacheProfiles` (`src/router.tsx`), so tune profiles there rather than per-call.
- **`cacheTag("policies", "privacy-policy")`** tags the entry so a webhook, mutation, or manual path can invalidate it later via `updateTag`/`revalidateTag`.
- Always pass `locale` and extract `country`/`language` from it

Cache behavior follows the consuming route, not a universal default. Use plain `"use cache"` for public identity/body content that must be coherent in the static shell. Use `"use cache: remote"` for reusable public results resolved after request inputs such as filters, search, or cursors. Keep cart, session, and Customer Account data request-scoped or private. Pair any cached operation with `cacheTag` so a webhook, mutation, or deliberate manual path can invalidate every associated tag.
...
Call the operation from a route handler — a server function that receives the request context and returns JSX:

```tsx
import { type Handler, notFound } from "@rangojs/router";

import { getPrivacyPolicy } from "@/lib/shopify/operations/policies";
import { getLocale } from "@/lib/params";

export const PrivacyPolicyPage: Handler<"/policies/privacy"> = async () => {
...
Then wire the handler into the route table in `src/urls.tsx` so it responds at a URL:

```tsx
path("/policies/privacy", PrivacyPolicyPage, { name: "privacyPolicy" }),
```
...
export async function updateCartNote(note: string, cartId: string) {
  const response = await storefront.request<{
    cartNoteUpdate: { cart: ShopifyCart };
  }>(UPDATE_CART_NOTE_MUTATION, { variables: { cartId, note } });
...
Expose mutations to the client through server actions in `lib/cart/action.ts` using `"use server"`.
...
- [ ] Cache directive (`"use cache"` / `"use cache: remote"`) chosen from the route's render role, with `cacheTag` for invalidation where applicable
