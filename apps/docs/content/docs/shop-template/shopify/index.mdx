...
For token setup and required scopes, see [Getting Started](/docs/shop-template/getting-started) and [Storefront API Permissions](/docs/shop-template/reference/storefront-api-permissions).

For the API client and query patterns, see [Storefront API](/docs/shop-template/reference/storefront-api).
...
By default, the template uses hardcoded navigation links and an empty footer. Use [`/vercel-shop:enable-shopify-menus`](/docs/shop-template/skills/enable-shopify-menus) to replace them with dynamic menus fetched from Shopify.
...
The handler relies on fine-grained tags: per-product (`product-{numericId}`, stamped on every list and collection read via `tagProducts()`) and per-collection (`collection-{handle}`, stamped on every collection read including the all-collections listing). Because create and delete change the _set_ of collections, they also fire `collections-index` — a tag carried only by the all-collections reads (`getCollections`, `getCollectionsListing`) and the collections sitemap — so the listing refreshes without touching other PLPs. The broad `products` and `collections` tags sit on every read of their type but are never fired automatically; they're manual break-glass levers (`revalidateTag("collections")`) for purging all data of a type at once. There is intentionally no `inventory_levels/*` branch. See [Webhooks](/docs/shop-template/anatomy/webhooks) for the full rationale.
...
| File                             | Purpose                                                                     |
| -------------------------------- | --------------------------------------------------------------------------- |
| `src/routes/api.tsx`             | `shopifyWebhook` response handler with HMAC verification                    |
| `src/urls.tsx`                   | Route table registering `path.any("/api/webhooks/shopify", shopifyWebhook)` |
