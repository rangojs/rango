---
title: Extending with Skills
description: Agent-ready skills for extending your Shopify storefront.
type: overview
prerequisites:
  - /docs/getting-started
---

Skills are agent-ready guides that transform your storefront. Run them with Claude Code or follow along manually.

<Cards>
  <Card
    title="Build with Vercel Shop"
    href="/docs/shop-template/skills/build-shop"
  >
    Build or adapt Shopify storefronts with source-backed Vercel Shop patterns.
  </Card>
  <Card
    title="Shopify GraphQL Integration"
    href="/docs/shop-template/skills/shopify-graphql-reference"
  >
    Apply Vercel Shop conventions after Shopify AI Toolkit validates an
    operation.
  </Card>
  <Card
    title="Shopify Markets"
    href="/docs/shop-template/skills/enable-shopify-markets"
  >
    Add multi-locale and multi-currency support with Shopify Markets.
  </Card>
  <Card
    title="Shopify CMS"
    href="/docs/shop-template/skills/enable-shopify-cms"
  >
    Wire Shopify metaobjects as the CMS for homepage and marketing page content.
  </Card>
  <Card
    title="Shopify Menus"
    href="/docs/shop-template/skills/enable-shopify-menus"
  >
    Replace hardcoded nav and footer with Shopify-powered menus, plus optional
    megamenu.
  </Card>
</Cards>
