...
2. If the Vercel Shop template source is available, inspect it before relying on this prose:
   ...
   - Route table and shell: `apps/template/src/urls.tsx` (the `urls()`/`path()` route table), `apps/template/src/router.tsx` (`createRouter`, cache profiles, middleware), `apps/template/src/document.tsx` (HTML document), `apps/template/src/routes/root-layout.tsx` (root chrome layout with `<Outlet />`), `apps/template/src/routes/home.tsx` (home page handler)
...
4. Read the `@rangojs/router` guides under `node_modules/@rangojs/router/skills/` for any API being changed (e.g. `skills/route` for the route table and handlers, `skills/caching` and `skills/use-cache` for `"use cache"` profiles, `skills/middleware`, `skills/handler-use` for the `Meta` handle, `skills/links` for prefetch). This template may use newer routing, caching, and prefetch behavior than model knowledge.
