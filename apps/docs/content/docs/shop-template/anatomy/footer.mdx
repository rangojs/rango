...
The footer always renders. By default it shows a copyright line ("© \{siteConfig.name\}. All rights reserved.") and social media icons when configured. The [`enable-shopify-menus`](/docs/shop-template/skills/enable-shopify-menus) skill adds link columns sourced from a Shopify `"footer"` menu above the copyright.
...
The copyright line uses the `footer.copyright` i18n key with `siteConfig.name` interpolated in. `siteConfig.name` reads from `VITE_SITE_NAME` (via `import.meta.env`) and falls back to `"Vercel Shop"` — set that env var to brand your storefront. To change the wording (not just the name), edit the `footer.copyright` entry in `lib/i18n/messages/en.json`. The footer intentionally has no year — reading `new Date()` inside a `"use cache"` scope would bake a stale year into the cached output; add a year only if you read it in an uncached segment (e.g. behind a Suspense boundary).
