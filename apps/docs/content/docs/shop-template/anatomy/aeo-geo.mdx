...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- |
| **Content negotiation**          | Serves clean markdown to AI clients via `Accept: text/markdown`                                        | `src/middleware.ts`, `src/routes/machine.tsx`, `lib/markdown/**`                |
...
| **Sitemap**                      | [Sitemap index + paged children](/docs/shop-template/anatomy/sitemap) for products and collections     | `src/routes/machine.tsx`, `src/urls.tsx`                                        |
| **Robots**                       | Declares crawl policy and blocks faceted (sort/filter) collection URLs                                 | `src/routes/machine.tsx`                                                        |
| **`llms.txt`**                   | Curated `/llms.txt` index of collections and discovery links for AI agents                             | `src/routes/machine.tsx`, `lib/markdown/llms.ts`                                |
| **OpenGraph & Twitter metadata** | Per-route title/description/image previews, plus `product` OG tags (type, price, availability) on PDPs | `components/product-detail/open-graph.tsx`, the `Meta` handle in `src/routes/*` |
...
This is built in and requires no configuration. The content-negotiation handlers are co-located in `src/routes/machine.tsx` and registered as `/md/*` response routes in `src/urls.tsx`, so they're easy to find.
...
GET /search?q=speaker        (Accept: text/markdown)
    ↓
markdownNegotiationMiddleware (src/middleware.ts)
    ↓
productMarkdown / collectionMarkdown / searchMarkdown
(the same handlers served at /md/products/:handle, /md/collections/:handle, /md/search)
    ↓
Response: text/markdown
```

Because Rango middleware can't rewrite the match target the way a config-based rewrite could, `markdownNegotiationMiddleware` detects the header on the canonical URL and dispatches directly to the matching markdown handler. Browsers that don't send `Accept: text/markdown` are unaffected — the middleware only dispatches to the markdown handler when the header matches.
...
| File                         | Purpose                                                                          |
| ---------------------------- | -------------------------------------------------------------------------------- |
| `src/middleware.ts`          | `markdownNegotiationMiddleware` — matches `Accept: text/markdown` and dispatches |
| `src/urls.tsx`               | Registers the `/md/*` response routes (`path.any(...)`)                          |
| `src/routes/machine.tsx`     | `productMarkdown`, `collectionMarkdown`, `searchMarkdown` handlers               |
...
If you have enabled Shopify Markets, `markdownNegotiationMiddleware` runs inside `localeMiddleware` (both chained on the router with `.use()`), so the request-scoped i18n context is established before the markdown handler renders — no additional configuration is needed. Pass a `?locale=` query parameter to the markdown endpoint for locale-specific pricing and collection/search context.
...
The route is dynamic: collections are pulled live from Shopify and capped so the file stays a concise index rather than an exhaustive dump — the full URL set lives in the [sitemap](/docs/shop-template/anatomy/sitemap). Because the links honor content negotiation, the virtual `/collections/all` catalog page is served as Markdown too, not just HTML.

### Key files

| File                     | Purpose                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------- |
| `src/routes/machine.tsx` | `llms` handler (registered in `src/urls.tsx`); fetches collections, serves `text/plain` |
