...
The agent is opt-in via `VITE_ENABLE_AGENT="1"` (read at build time through `import.meta.env` and exposed as `agent.enabled` in `lib/config.ts`). When unset, the agent button is hidden from the storefront and the `/api/chat` route returns 404.
...
`/api/chat` ships without bot, rate-limit, or token-cap protection by default — it's reachable anonymously. Before exposing the agent to real traffic, harden it. A reasonable baseline on Vercel:

- **[Vercel BotID](https://vercel.com/docs/botid)** — initialize BotID in the browser with `initBotId({ protect: [{ path: "/api/chat", method: "POST" }] })` from a client entry point (e.g. a `"use client"` module loaded by the root layout), and call `checkBotId()` at the top of `chatHandler`. Returns 403 when a bot is detected and works locally without a Vercel deployment.
- **Per-IP rate limit** — a sliding-window log keyed by the client IP, backed by [Vercel Runtime Cache](https://vercel.com/docs/runtime-cache) (`getCache` from `@vercel/functions`) so it shares state across regions. Run it before `request.json()` and before any Shopify cart is created so bot floods don't poison cart records or burn gateway spend.
...
| File                               | Purpose                                                                                     |
| ---------------------------------- | ------------------------------------------------------------------------------------------- |
| `src/routes/chat.tsx`              | `chatHandler` — page context, cart init, streaming (wired to `/api/chat` in `src/urls.tsx`) |
| `lib/agent/server.ts`              | Agent creation, system prompt, AsyncLocalStorage context                                    |
| `lib/agent/tools/`                 | Tool implementations using Shopify operations                                               |
| `components/agent/agent-panel.tsx` | Chat panel UI                                                                               |
| `components/agent/registry.tsx`    | Generative UI component registry                                                            |

Auxiliary components (`client.tsx`, `agent-button.tsx`, `cart-reconciler.tsx`) and the json-render schemas in `lib/agent/index.ts` live next to these.
