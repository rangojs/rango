...
The navigation system is split into a sticky top navbar for desktop and a fixed bottom bar for mobile. By default, navigation links are hardcoded. Use [`/vercel-shop:enable-shopify-menus`](/docs/shop-template/skills/enable-shopify-menus) to fetch menus from Shopify.
...
The cart icon is a parallel-slot widget. The route tree registers a `@navCart` slot with `NavCartLoader` (which reads the `shopify_cartId` cookie and fetches the cart) and a `loading()` fallback; the navbar renders it with `<ParallelOutlet name="@navCart" />`. The client component consumes the loader with `useLoader(NavCartLoader)` and renders a quantity badge with an animation for pending additions, opening the cart overlay on click.

The loader-behind-`loading()` shape is what keeps the widget out of [PPR shells](/docs/shop-template/anatomy/ppr): the cookie read runs fresh on every request while the nav chrome around it is served from the cached shell. The account icon (`@navAccount` slot, `NavSessionLoader`) follows the same pattern for the session read.
...
| `components/nav/loaders.ts`       | Live-lane loaders for the cart and account widgets |
...
`quick-links`, `cart` / `cart-client`, and the footer components sit in their respective directories; the [footer page](/docs/shop-template/anatomy/footer) covers footer composition.
