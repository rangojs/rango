...
<Note>
  The cart ID is stored in a `shopify_cartId` cookie with a 7-day expiry.
  Sessions longer than 7 days start a fresh cart.
</Note>
...
| `components/cart/overlay.tsx`    | Cart slideover composition                                      |
| `app/cart/page.tsx`              | Full cart page route                                            |
| `lib/cart/action.ts`             | Server actions for cart mutations                               |
| `lib/cart/server.ts`             | `invalidateCartCache()` — must be called by every cart mutation |
