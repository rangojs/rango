...
The loader-behind-`loading()` shape is what keeps the widget out of [PPR shells](/docs/anatomy/ppr): the cookie read runs fresh on every request while the nav chrome around it is served from the cached shell. The account icon (`@navAccount` slot, `NavSessionLoader`) follows the same pattern for the session read.
