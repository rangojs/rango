...
export function Comments({ slug }: CommentsProps) {
  ...
  return (
    <section data-testid="comments-section" style={{
      marginTop: "2rem", borderTop: "1px solid #e5e7eb", paddingTop: "1.5rem",
    }}>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            fontSize: "0.9rem", fontWeight: 500, alignSelf: "flex-start",
          }}
        >
      ...
      {comments.length === 0 ? (
        <p data-testid="no-comments" style={{ color: "#9ca3af", fontStyle: "italic" }}>
          No comments yet. Be the first!
        </p>
      ) : (
        <div data-testid="comments-list" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {comments.map((comment) => (
            <div key={comment.id} data-testid={`comment-${comment.id}`} style={{
              ...
              <p style={{ color: "#374151", lineHeight: 1.5, fontSize: "0.9rem", margin: 0 }}>
                {comment.text}
              </p>
            </div>
          ))}
