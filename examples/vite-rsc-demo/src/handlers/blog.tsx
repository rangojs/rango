...
export default map<typeof blogRoutes>({
  ...
  index: () => (
    <div>
      ...
      <ul>
        ...
        <li><a href="/blog/react-server-components">React Server Components</a></li>
        <li><a href="/blog/router-design">Router Design</a></li>
      </ul>
    </div>
  ),
  ...
  post: (ctx) => (
    <div>
      <h2>{ctx.params.slug.split('-').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}</h2>
      <p className="segment-id">Segment: Blog Post Route</p>
      <p><strong>Slug:</strong> <code>{ctx.params.slug}</code></p>
