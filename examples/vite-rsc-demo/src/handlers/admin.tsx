import { map, layout, revalidate, type RevalidateParams } from "rsc-router";
...
export default map<typeof adminRoutes>({
  ...
  // ===================================================
  // SOFT REVALIDATION - Global Default (runs for all routes)
  // ===================================================

  // Global soft decision: suggests revalidation but allows override
  [revalidate("*", "global-default")]: () => {
  ...
  [revalidate("user")]: ((params: RevalidateParams<{ id: string }>) => {
  ...
  // Users list: No specific revalidator
  // → Falls through to global soft decision
  // → Final result: true (from global)
