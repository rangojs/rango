...
devTest.describe("blog-author-intercept", () => {
  devTest("should show author modal when clicking author from blog index", async ({
    page,
    devServerURL,
  }) => {
  ...
  devTest("should navigate directly to author page from a post", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await expect(page.locator('[data-testid="author-page"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator("h2:has-text('Jane Doe')")).toBeVisible();
  });

  devTest("should close author modal on back navigation", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
  ...
  devTest("should navigate from author modal to full author page", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
...
devTest.describe("blog-author-breadcrumbs", () => {
  devTest("should display correct breadcrumbs on author page (direct nav)", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await expect(page.locator("h2:has-text('Jane Doe')")).toBeVisible({ timeout: 5000 });
  ...
  devTest("should display correct breadcrumbs on author posts page", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await expect(page.locator("h2:has-text('Posts by Jane Doe')")).toBeVisible({ timeout: 5000 });
  ...
  devTest("should stream async breadcrumb content on author page", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await expect(breadcrumbNav.locator("text=(2 posts)")).toBeVisible({ timeout: 5000 });
  });

  devTest("should update breadcrumbs through deep navigation: index -> post -> author -> posts", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await page.locator('a[href="/blog/author/jane-doe/posts"]').first().click();
    await expect(page.locator("h2:has-text('Posts by Jane Doe')")).toBeVisible({ timeout: 5000 });
    await expect(breadcrumbNav.locator("text=Blog")).toBeVisible();
    await expect(breadcrumbNav.locator("text=Jane Doe")).toBeVisible();
    await expect(breadcrumbNav.locator("text=Posts")).toBeVisible();
  ...
  devTest("should update breadcrumbs when navigating from modal to full author page", async ({
    page,
    devServerURL,
  }) => {
    using _ = expectNoPageError(page);
    ...
    await page.locator("text=View Full Details").click();
    await expect(page.locator("text=Intercepted")).not.toBeVisible({ timeout: 3000 });
    await expect(page.locator("h2:has-text('Jane Doe')")).toBeVisible({ timeout: 10000 });
...
test.describe("blog-author-breadcrumbs (production)", () => {
  ...
  test("should display correct breadcrumbs on author page", async ({ page }) => {
    using _ = expectNoPageError(page);
  ...
  test("should display correct breadcrumbs on author posts page", async ({ page }) => {
    using _ = expectNoPageError(page);

    await page.goto(f.url("/blog/author/jane-doe/posts"));
    await waitForHydration(page);

    await expect(page.locator("h2:has-text('Posts by Jane Doe')")).toBeVisible({ timeout: 10000 });
  ...
  test("should show author modal from index in production", async ({ page }) => {
    using _ = expectNoPageError(page);
  ...
  test("should navigate directly to author from post in production", async ({ page }) => {
    using _ = expectNoPageError(page);

    await page.goto(f.url("/blog/hello-world"));
    await waitForHydration(page);

    await expect(page.locator("h2:has-text('Hello World')")).toBeVisible({ timeout: 10000 });
