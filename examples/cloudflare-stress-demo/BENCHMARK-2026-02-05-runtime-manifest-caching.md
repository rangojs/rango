...
export async function getRouteManifestData(
  ...
  version: string,
  options?: { store?: SegmentCacheStore; waitUntil?: (p: Promise<void>) => void }
  ...
  if (store) {
    ...
    if (cached?.manifest?.version === version) {
      console.log(`[route-manifest] HIT edge cache (${duration}ms, ${routes} routes)`);
  ...
  console.log(`[route-manifest] MISS - generated fresh (${duration}ms, ${routes} routes)`);
