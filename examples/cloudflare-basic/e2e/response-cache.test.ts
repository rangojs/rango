...
test.describe("response-cache (dev)", () => {
  ...
  test("control: uncached route returns different timestamps", async ({ request }) => {
  ...
  test("path.json() with cache() returns identical timestamp on cache hit", async ({ request }) => {
  ...
  test("path.text() with cache() returns identical body on cache hit", async ({ request }) => {
  ...
  test("path.xml() with cache() returns identical body on cache hit", async ({ request }) => {
  ...
  test("path.html() with cache() returns identical body on cache hit", async ({ request }) => {
...
test.describe("response-cache (production)", () => {
  ...
  test("path.text() with cache() returns identical body on cache hit in production", async ({ request }) => {
    const res1 = await request.get(f.url("/test/cached-text"));
    expect(res1.status()).toBe(200);
    expect(res1.headers()["content-type"]).toContain("text/plain");
    const body1 = await res1.text();
