...
test.describe("MIME type response routes (dev)", () => {
  ...
  test("path.json() returns application/json with envelope", async ({ request }) => {
  ...
  test("path.image() returns image content-type from handler Response", async ({ request }) => {
  ...
  test("path.stream() returns stream content-type from handler Response", async ({ request }) => {
  ...
  test("path.any() returns custom content-type from handler Response", async ({ request }) => {
    const response = await request.get(f.url("/test/mime/any"));
    expect(response.status()).toBe(200);
    expect(response.headers()["content-type"]).toContain("application/x-custom");
    const body = await response.text();
    expect(body).toBe("custom");
  });

  test("all MIME routes skip RSC pipeline (no Flight content-type)", async ({ request }) => {
    const paths = [
      "/test/mime/json",
      "/test/mime/text",
      "/test/mime/html",
  ...
  test("partial request to any MIME route returns X-RSC-Reload", async ({ request }) => {
    const paths = [
      "/test/mime/json",
      "/test/mime/text",
      "/test/mime/html",
...
test.describe("MIME type response routes (production)", () => {
  ...
  test("path.json() returns application/json in production", async ({ request }) => {
    const response = await request.get(f.url("/test/mime/json"));
    expect(response.status()).toBe(200);
    expect(response.headers()["content-type"]).toContain("application/json");
    const body = await response.json();
  ...
  test("path.xml() returns application/xml in production", async ({ request }) => {
    const response = await request.get(f.url("/test/mime/xml"));
    expect(response.status()).toBe(200);
    expect(response.headers()["content-type"]).toContain("application/xml");
  });
  ...
  test("path.stream() returns octet-stream in production", async ({ request }) => {
    const response = await request.get(f.url("/test/mime/stream"));
    expect(response.status()).toBe(200);
    expect(response.headers()["content-type"]).toContain("application/octet-stream");
  });

  test("path.any() returns custom content-type in production", async ({ request }) => {
    const response = await request.get(f.url("/test/mime/any"));
    expect(response.status()).toBe(200);
    expect(response.headers()["content-type"]).toContain("application/x-custom");
  });
