...
test.describe("API demo - typed fetch with PathResponse", () => {
  ...
  test("should display fetched product detail", async ({ page }) => {
    ...
    await expect(testId(page, "product-detail-id")).toBeVisible({ timeout: 10000 });
    ...
    await expect(testId(page, "product-detail-description")).toContainText("Details for product 1");
  });
