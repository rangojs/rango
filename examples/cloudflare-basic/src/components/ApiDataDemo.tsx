...
export function ApiDataDemo() {
  const [health, setHealth] = useState<PathResponse<"/api/health"> | null>(null);
  const [products, setProducts] = useState<PathResponse<"/api/products"> | null>(null);
  const [product, setProduct] = useState<PathResponse<"/api/products/:id"> | null>(null);

  useEffect(() => {
    async function load() {
      const healthResult: PathResponse<"/api/health"> = await fetch(href("/api/health")).then((r) => r.json());
      setHealth(healthResult);

      const productsResult: PathResponse<"/api/products"> = await fetch(href("/api/products")).then((r) => r.json());
      setProducts(productsResult);

      const productResult: PathResponse<"/api/products/:id"> = await fetch(href("/api/products/1")).then((r) => r.json());
        ...
        {health ? (
          health.error ? (
            ...
            <>
              ...
              <span data-testid="health-timestamp">{health.data.timestamp}</span>
            </>
          )
        ) : (
        ...
        {product ? (
          product.error ? (
            ...
            <>
              ...
              <span data-testid="product-detail-price">{product.data.price}</span>
              <span data-testid="product-detail-description">{product.data.description}</span>
            </>
          )
        ) : (
