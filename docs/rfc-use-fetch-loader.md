...
export const ProtectedLoader = createLoader(async (ctx) => {
...
```typescript
export const FileLoader = createLoader(async (ctx) => {
...
export function useFetchLoader<T>(loader: LoaderDefinition<T>) {
  ...
  const load = useCallback(async (options?: LoadOptions) => {
    if (!loader.$$id) {
      throw new Error(`Loader is missing $$id. Ensure exposeLoaderId plugin is enabled.`);
    }
