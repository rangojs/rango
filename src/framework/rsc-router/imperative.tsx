 ...
 * Legacy imperative API - maintained for backward compatibility
 */
export class RscRouter {
  ...
  async match(request: Request): Promise<[ReactNode | null, Segment[]]> {
    ...
    // Execute middleware and handlers with proper chaining
    ...
    // Build handler chain from all matched routes
    const allHandlers: Array<{
      handler: MiddlewareHandler | RouteHandler | LayoutHandler;
      route: Route;
      index: number;
    }> = [];

    // Collect all handlers from outermost to innermost
    for (let i = matchedRoutes.length - 1; i >= 0; i--) {
      const { route } = matchedRoutes[i];
      for (const handler of route.handlers) {
        allHandlers.push({ handler, route, index: i });
      }
    }

    // Create a chain of handlers
    const executeHandlerChain = async (handlerIndex: number): Promise<void> => {
      if (handlerIndex >= allHandlers.length) {
        return;
      }

      const { handler, route, index } = allHandlers[handlerIndex];

      console.log(
        `[Router.match] Processing handler ${handlerIndex}: ${
          handler.name || "anonymous"
        }, length: ${handler.length}, pattern: ${route.pattern}`
      );
      ...
      if (route.isLayout) {
        ...
        await executeHandlerChain(handlerIndex + 1);
        ...
        console.log(
          ...
          typeof componentTree === 'object' && componentTree && 'type' in componentTree
            ? (componentTree as any).type?.name
            : typeof componentTree
        );
        // Route handlers don't need to call next
      }
    };

    // Start the handler chain
    await executeHandlerChain(0);
