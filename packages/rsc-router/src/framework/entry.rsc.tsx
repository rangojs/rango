...
async function _handler(request: Request, router: RSCRouter): Promise<Response> {
    // ========================================================================
    ...
    if (isPartialRequest && previousPathname) {
      ...
      if (partialResult && partialResult.segments.length > 0) {
        ...
        console.log(`[RSC]   Preserved layouts: ${preservedLayouts.join(', ') || 'none'}`);
        ...
        const segmentMetadata: SegmentMetadata[] = segments.map(toSegmentMetadata);
        ...
        if (match && (match as any).matched) {
          const { buildSegmentMap, renderSegments } = await import('../segment-system');
