 ...
 */
export const parallelMetaPatterns = urls(
  ({ path, layout, parallel, cache }) => [
    // cache() wraps all routes — null-component parallels (@meta returning null)
    // and void route handlers must NOT block cache writes.
    cache({ ttl: 600 }),
    layout(
      (ctx) => {
        ...
        path(
          ...
          (ctx) => (
            <div data-testid="pm-product-page">
              ...
              <span data-testid="pm-render-ts">{Date.now()}</span>
            </div>
          ),
          { name: "product" },
          () => [
