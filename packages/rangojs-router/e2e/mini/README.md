...
(`router.tsx` + `client.tsx`) hold the bulk; a few small modules exist only
because of RSC's module-directive boundary (`actions.tsx`, `shared.tsx`) and
because `useReverse` needs a per-module routes map (`urls/products.tsx`) — none
of them a limitation of the router itself.
...
| `src/urls/products.tsx` | _(server)_     | A route **module** included via `include("/products", …)`: the products group (params, parallel `@cart`, intercept modal, `transition()`, the product catalog + its `ctx.use()` loader). It lives in its own file **only because `useReverse` needs a per-module routes map** (see "What doesn't fit").                                     |
...
So the honest shape is **2 main files + 2 RSC-mandated modules**, plus **one route
module (`urls/products.tsx`) that exists only because `useReverse` needs its own
per-module file**. There is no `index.html`, no client entry, and no server
entry — the Vite plugin injects the browser/SSR/RSC entries automatically.
...
- **Server-only definitions need no separate module.** The `urls()` patterns
  ...
  of definitions the client touches by identity. (The products module is the one
  exception, and not for RSC reasons — see below.)
...
`useReverse` (via the `urls/products.tsx` module).
Head: `Meta` (title template) + `Breadcrumbs` + `useHandle`, `MetaTags` via the
default document.

## What deliberately does **not** fit the single-file shape

- **`useReverse` needs its own route module** (now exercised, not skipped). It
  imports a per-module `urls/*.gen.ts`, which only exists for a route group in
  its **own** `urls/*.tsx` file (and is emitted by the `rango generate` CLI, not
  the dev/build plugin). An inline `include()` group gets global named routes
  (`ctx.reverse`, `href`, `Link`) but **no** client reverse map. So to exercise
  `useReverse` we extracted the products group into `urls/products.tsx` — that
  extraction _is_ the demonstration of this constraint. Everything else —
  `Link`, `href`, `ctx.reverse`, `useHref`, `useMount`, `useLinkStatus` — works
  inline with no extra module.
