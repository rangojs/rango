...
test.describe("prerender-intercept (production build)", () => {
  ...
  test("different params render different detail content from prerender store", async ({
    ...
    expect(alphaTime).toMatch(/^\d+$/);
    expect(betaTime).toMatch(/^\d+$/);
  });
