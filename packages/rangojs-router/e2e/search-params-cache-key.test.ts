...
/**
 * Global `cache.searchParams` key filtering (router.tsx sets
 * `searchParams: { exclude: ["utm_*", "x_e2e_excluded"] }`).
 *
 * Strategy (same as response-cache.test.ts): the /spk/cached handler embeds
 * Date.now(); an identical timestamp across requests proves both URLs
 * resolved to ONE cache slot, differing timestamps prove distinct slots.
 */
...
function defineSearchParamsKeySpec(f: { url: (path: string) => string }) {
  test("excluded utm_* variants collapse onto one cache slot", async ({
    ...
    const res1 = await request.get(f.url("/spk/cached?utm_source=first"));
    ...
    // Poll until the async cache write (waitUntil) lands, then a DIFFERENT
    // excluded value must be a HIT on the same slot -- byte-identical body,
    // including the first request's baked-in utm value.
    await expect(async () => {
      const res2 = await request.get(f.url("/spk/cached?utm_source=second"));
    ...
    const res3 = await request.get(f.url("/spk/cached"));
  ...
  test("a non-excluded param still keys a distinct slot", async ({
    ...
    const res1 = await request.get(f.url("/spk/cached?page=1"));
    expect(res1.status()).toBe(200);
    const body1 = await res1.json();
    ...
    await expect(async () => {
      const again = await request.get(f.url("/spk/cached?page=1"));
      expect((await again.json()).ts).toBe(body1.ts);
    }).toPass({ timeout: 5_000 });

    const res2 = await request.get(f.url("/spk/cached?page=2"));
    const body2 = await res2.json();
    ...
    await expect(async () => {
      const res4 = await request.get(
        f.url("/spk/cached?page=1&utm_source=ride"),
      );
