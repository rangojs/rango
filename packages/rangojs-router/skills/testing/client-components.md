...
| `defaultPrefetch` | `PrefetchStrategy`                                                     | Router default to apply while mounted. Mirrors `createRouter({ defaultPrefetch })` for `<Link>` and plain anchors unless they set `data-prefetch="false"`.                                                           |
