...
| `defaultPrefetch` | `PrefetchStrategy`                                                     | Router default to apply while mounted. Mirrors `createRouter({ defaultPrefetch })` for `<Link>` and opted-in plain anchors (`data-prefetch="true"`).                                                                 |
