...
| `defaultPrefetch` | `PrefetchStrategy`                                                     | Instance-scoped router default. Mirrors `createRouter({ defaultPrefetch })` for `<Link>` and eligible basename-scoped plain anchors. `data-prefetch="false"`/`"none"` opts out; `"true"` allows routed resource suffixes. |
