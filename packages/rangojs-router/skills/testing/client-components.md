...
| `defaultPrefetch` | `PrefetchStrategy`                                                     | Instance-scoped router default. Mirrors `createRouter({ defaultPrefetch })` for `<Link>` and basename-scoped, non-resource plain anchors unless they set `data-prefetch="false"`.                                    |
