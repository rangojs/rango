...
This is the contributor-facing policy for deciding where an export belongs. When
you add or move one, the question is always the same: which public subpath would
a user reach for to find this? The goal is that every public subpath exposes
coherent user concepts, not implementation plumbing.
...
The root entrypoint is conditionally resolved through the `react-server` export
condition, so don't treat it as a generic client/runtime barrel.
...
The test: if an API is primarily used from client components, it belongs here.
...
`./cache` is the canonical user-facing home, so don't expose cache stores from
`./rsc` or other advanced subpaths.
...
Do not expose raw context objects or script-generation internals unless they are
...
These are advanced server-only integration subpaths. Use them for:

- custom request-pipeline APIs
- custom HTML rendering bridge APIs
- their public option and dependency types

The temptation is to treat them as overflow buckets — don't. Keep unrelated
public utilities out, along with internal request-context mutation helpers,
handle-store plumbing, and duplicate cache exports.
...
runtime integration code but are not part of the public API contract. Examples:
...
Use `./__internal` for implementation plumbing that has no user-facing story and
can change freely.

## Review Checklist

When you add or move an export, run these four:
...
The boundary isn't just convention — it's enforced by tests, so a misplacement
gets caught:
...
If you change exports or import guidance, update the tests and the public docs in
the same PR.
