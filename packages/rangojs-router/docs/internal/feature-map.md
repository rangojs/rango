...
| `./testing/flight`          | Real Flight (RSC) rendering, `react-server` condition only: `renderToFlightString`, `normalizeFlight`, `assertFlightRuntimeAvailable`. Separate entry because its vendored react-server-dom serializer loads only under that node condition. A thrown Server Component surfaces as a rejected promise (the stream still completes), not a hang.                                                                                                                                                                                                                                                                                                                                 |
