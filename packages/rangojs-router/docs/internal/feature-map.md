...
- Advanced: `runInRequestContext` (build + enter a real ctx in one call — for an action/fn that reads `getRequestContext()`/`cookies()` but has no loader-context shape), `runWithRequestContext` (re-export; enter a ctx already built), `createTestRequestContext`, `toRequest`, `seedVariables`.
