...
- Integration: `dispatch` (`./testing`; router -> Response for redirects/404/response routes/middleware short-circuits, NO Flight; mirrors `handleResponseRoute` error + `Vary: Accept` semantics; throws on RSC routes; the router transitively imports `@vitejs/plugin-rsc/rsc`, so tests need a Vite-RSC env or `vi.mock`). `renderRoute` (`./testing/dom`; RTL-style client-tree stub; mounts the real `NavigationProvider`; seeds `loaderData` by `$$id`; client-tree fidelity only).
