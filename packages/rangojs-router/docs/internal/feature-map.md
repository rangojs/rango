...
- Advanced: `runInRequestContext` (build + enter a real ctx in one call — for an action/fn that reads `getRequestContext()`/`cookies()` but has no loader-context shape; returns `{ result, response, cookies, locationState }` (type `RunInRequestContextResult`) so an action's Set-Cookie / flash output is assertable without the `@internal` `ctx.res`/`ctx.cookies()` cast), `runWithRequestContext` (re-export; enter a ctx already built), `createTestRequestContext`, `toRequest`, `seedVariables`.
