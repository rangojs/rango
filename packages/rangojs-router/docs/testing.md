...
| `ppr` shell (axis 2)                             | document HIT/MISS; partial-navigation replay HIT; store key / getShell after flush  | e2e + signal / unit | `assertShellStatus` / `assertPprReplayStatus` / `shellCacheKey` + `MemorySegmentCacheStore`                    | `/ppr`                                   |
...
server-side effect under the primitives — the rotation `Set-Cookie` (the state
cookie name is seeded by default, customizable via the **`stateCookie`** option
and surfaced as `result.stateCookieName`) and the `x-rango-keep-cache` directive
header — assertable on `result.response` / `result.headers` (see `renderHandler`
/ `runInRequestContext`). Likewise server-side **cache-tag invalidation**
(`updateTag()`) evicts a tagged entry in-process against a
`MemorySegmentCacheStore` seeded via `cacheStore`, so read-your-own-writes is
unit-assertable. What stays e2e is whether the CLIENT bridge ACTS on these
signals (the prefetch wipe / SWR refetch / cross-tab broadcast) and the
distributed CDN/KV tag purge that needs `CFCacheStore`.
...
runInRequestContext<T>(fn: (ctx) => T | Promise<T>, opts?):
  ...
  // stateCookieName = the resolved rango state cookie name (assert an invalidateClientCache rotation against it).
