...
| importing your **whole app router** (`createRouter().routes(allRoutes)`) into a bare test                                                   | a router using `Prerender()` / `createLoader()` throws **"missing `$$id`"** — those ids are plugin-injected at build and absent in bare vitest                                                                                                                                                                                                        | Build the router from a **`Prerender`-free include** (your API/response routes) for `dispatch` / `assertGeneratedRoutesMatch`; test prerender/loader-bearing routes at **e2e** (see Setup → "Resolving `@rangojs/router`") |
