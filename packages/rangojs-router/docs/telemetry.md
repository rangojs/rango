...
| `handler:{id}`                                         | Handler    | Per-segment route/layout handler EXECUTION (the component/handler that produces a segment) — the dominant per-segment work. Co-emitted with the `rango.handler` span (`rango.segment_id={id}`); the metric is owned by `track()` at the call site, the span by `observePhase`. Server-Timing prefixes the depth (`d2-handler-{id}`) |
