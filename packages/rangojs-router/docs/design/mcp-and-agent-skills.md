...
Phase 0 established the route/discovery transport. `rango mcp` is the stdio
connector and discovers the server through an owner-only descriptor under
`~/.rango/mcp`.
...
Phase 1 added a development-only runtime diagnostic core. One server-owned
...
when the application has no `TelemetrySink`.
...
### Phase 3: render explanation (implemented)
...
### Phase 4: workflow skills (implemented)
...
The published workflow documents follow a shared context, evidence, edit,
verification, stop-conditions, and report order. The skills guard also requires
each workflow's fixture task to state a broken state, expected diagnosis/edit,
and separate development and production verification.
