...
export function createHandlerContext<TEnv>(
      ...
      if (typeof value === "function") {
        if (MUTATING_HEADERS_METHODS.has(prop as string)) {
          return (...args: any[]) => {
            ...
            assertNotInsideCacheScope(ctx, "headers");
  ...
  ctx = {
    ...
    var: variables,
    get: ((keyOrVar: any) => {
      // Guard: reading a non-cacheable var inside cache scope throws
      if (
        isContextVar(keyOrVar) &&
        isNonCacheable(variables, keyOrVar) &&
        ((INSIDE_CACHE_EXEC as symbol) in ctx ||
          (INSIDE_CACHE_SCOPE as symbol) in ctx)
      ) {
        const scope =
          (INSIDE_CACHE_SCOPE as symbol) in ctx ? "cache()" : '"use cache"';
        throw new Error(
          `ctx.get() for a non-cacheable variable cannot be called inside a ${scope} boundary. ` +
    ...
    set: ((keyOrVar: any, value: any, options?: ContextSetOptions) => {
      ...
      // Guard: setting a non-cacheable var inside cache() scope throws.
      // "use cache" already blocks ALL ctx.set() via assertNotInsideCacheExec.
      // For cache(), only non-cacheable vars are blocked.
      if (
        (INSIDE_CACHE_SCOPE as symbol) in ctx &&
        isContextVar(keyOrVar) &&
        (!keyOrVar.cache || options?.cache === false)
      ) {
        throw new Error(
          `ctx.set() with a non-cacheable variable cannot be called inside a cache() boundary. ` +
            `The variable was created with { cache: false } or this write uses { cache: false }. ` +
            `On cache hit the handler is skipped, so the value would be lost. ` +
            `Move ctx.set() to a middleware or layout outside the cache() scope.`,
        );
      }
