...
async function driveFreshIntercept(status: number) {
  const layout = seg("R0");
  ...
  await runWithRouterContext(makeCacheStoreRouterContextStub(), () =>
    runWithRequestContext(reqCtx, async () => {
      ...
      // Fire the onResponse callback; a 200 schedules the cache write.
...
describe("withCacheStore — intercept segment dedup (B4)", () => {
  it("caches each segment once on a fresh intercept navigation", async () => {
    const cacheRoute = await driveFreshIntercept(200);
  ...
  it.each([500, 404])(
    "does not cache a %i intercept navigation (handler error / notFound fallback)",
    async (status) => {
      const cacheRoute = await driveFreshIntercept(status);
