...
export async function executeMiddleware<TEnv>(
  ...
  const next = async (): Promise<Response> => {
    ...
    // Wrap the middleware (including its downstream next() chain) in a span.
    // Spans nest by async context, so this onions middleware-over-middleware
    // and the core handler underneath. Pass-through when tracing is off.
    ...
    try {
      result = await traceSpan(
        ...
        "rango.middleware",
        (span) => {
          span.setAttribute("rango.middleware_name", metricLabel);
          return entry.handler(ctx, wrappedNext);
        },
      );
    } catch (error) {
