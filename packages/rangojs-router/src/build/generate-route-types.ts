...
export function writeCombinedRouteTypes(root: string, knownRouterFiles?: string[]): void {
  ...
  for (const routerFilePath of routerFilePaths) {
    ...
    if (existing !== source) {
      // Don't overwrite a file from runtime discovery (which has all dynamic
      // routes) with a smaller set from the static parser. The static parser
      // can't see routes generated by Array.from() or other dynamic code.
      if (existing) {
