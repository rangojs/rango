...
async function createClient(): Promise<Client> {
  ...
  const server = createRangoMcpServer("1.0.0", {
    ...
    matchRoute: (input) => jsonToolResult({ input, matched: false }),
    ...
    listNavigations: (input) => jsonToolResult({ input, navigations: [] }),
    getNavigationTrace: (input) => jsonToolResult({ input, navigation: null }),
