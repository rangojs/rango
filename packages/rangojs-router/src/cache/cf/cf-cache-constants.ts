...
// ============================================================================
//
// Standalone cache header-name constants, tag-marker prefixes, intervals, and
// latency timeouts extracted from cf-cache-store.ts. This module must not import
// the CFCacheStore class (avoids a circular import). The public constants are
// re-exported from cf-cache-store.ts to preserve the import path; the private
// ones (CACHE_EXPIRES_AT_HEADER / CACHE_ORIG_CC_HEADER / TAG_MARKER_CACHE_PREFIX
// / TAG_MARKER_ABSENT) are imported there for internal use only.
