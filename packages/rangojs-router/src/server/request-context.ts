...
> {
  ...
  /** @internal Router context (closure functions from createRouter). Set via derived ALS snapshot in runWithRouterContext. */
  _router?: any;

  /** @internal True when executing inside a loader scope (cache guards bypassed). Set via derived ALS snapshot in runInsideLoaderScope. */
  _isInsideLoader?: boolean;

  /** @internal Debug log context (request ID, transaction ID, depth). Set via derived ALS snapshot in runWithRouterLogContext/withRouterLogScope. */
  _logContext?: {
    requestId: string;
    transactionId: string;
    depth: number;
    revalidationTrace?: any;
  };

  /**
   * @internal Shared mutable state that survives across ALS-derived snapshots.
   *
   * Derived snapshots (`{ ...ctx, _isInsideLoader: true }`) copy primitive
   * fields by value — mutations to the snapshot are lost when it unwinds.
   * This object is shared by reference, so mutations are visible to all
   * snapshots and the original context within a request.
   *
   * Contains fields set after context creation (e.g. after route matching).
   */
  _shared: RequestSharedState;
}

/** @internal Mutable state shared across all ALS snapshots within a request. */
export interface RequestSharedState {
...
// Registered on globalThis so context.ts can access it without circular imports.
// Exported as _requestContextStorage for router-context.ts and logging.ts.
const REQUEST_CONTEXT_KEY = Symbol.for("rangojs-router:request-context");
const requestContextStorage: AsyncLocalStorage<RequestContext<any>> = ((
  globalThis as any
)[REQUEST_CONTEXT_KEY] ??= new AsyncLocalStorage<RequestContext<any>>());

/** @internal The canonical request-scoped ALS. Only for derived snapshot .run() calls in router internals. */
export { requestContextStorage as _requestContextStorage };
...
export function setRequestContextParams(
  ...
  if (ctx) {
    // Write to _shared so mutations survive across ALS-derived snapshots
    ctx._shared.params = params;
    // Also set on ctx directly for backward compat with direct reads
    ctx.params = params;
    if (routeName !== undefined) {
      ctx._shared._routeName = routeName;
      ...
      ctx._shared.routeName = publicName;
      ctx.routeName = publicName;
    }
    // Update reverse with scoped resolution now that route is known
    const newReverse = createReverseFunction(
      getGlobalRouteMap(),
      routeName,
      params,
      routeName ? isRouteRootScoped(routeName) : undefined,
    );
    ctx._shared.reverse = newReverse;
    ctx.reverse = newReverse;
  }
}
...
export function setRequestContextPrevRouteKey(
  ...
  if (ctx && prevRouteKey !== undefined) {
    ctx._shared._prevRouteKey = prevRouteKey;
...
export function getLocationState(): LocationStateEntry[] | undefined {
  ...
  return ctx?._shared?._locationState;
}
...
export function createRequestContext<TEnv>(
  ...
  const ctx: RequestContext<TEnv> = {
    ...
    setLocationState(entries: LocationStateEntry | LocationStateEntry[]): void {
      ...
      // Write to _shared so mutations survive across ALS-derived snapshots
      this._shared._locationState = this._shared._locationState
        ? [...this._shared._locationState, ...arr]
        : arr;
    },
    ...
    // Shared mutable state — survives across ALS-derived snapshots
    _shared: {
      params: {},
  ...
  // Initialize _shared.reverse with the same initial reverse function
  ctx._shared.reverse = ctx.reverse;
