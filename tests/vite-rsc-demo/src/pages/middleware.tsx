    ...
    <DebugSegmentWrapper type="layout" name="Middleware Demo">
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "2rem" }}>
        ...
        >
          ...
          <Link
            to={reverse("middleware.dashboard")}
            style={{
              padding: "0.5rem 1rem",
              background: "#0d9488",
              color: "white",
          ...
          <Link
            to={reverse("middleware.user", { userId: "123" })}
            style={{
              padding: "0.5rem 1rem",
              background: "#6366f1",
              color: "white",
    ...
    <DebugSegmentWrapper type="route" name="Dashboard (Protected)">
      <div>
        ...
        <div style={{ marginTop: "1rem" }}>
          <Link
            to={`${reverse("middleware.dashboard")}?auth=true`}
            style={{
    ...
    <DebugSegmentWrapper type="route" name="API (Loader Middleware)">
      <div>
        ...
        <p style={{ marginTop: "1rem", color: "#6b7280" }}>
          Check the server console to see the middleware logging when this page
          loads.
        </p>
      </div>
    </DebugSegmentWrapper>
...
export const globalMiddleware: Middleware[] = [
  async (ctx, next) => {
    ...
    console.log(
      `[Middleware Demo] Request ${requestId} completed in ${elapsed.toFixed(2)}ms`,
    );
  },
];

// Dashboard middleware
export const dashboardMiddleware: Middleware[] = [
  async (ctx, next) => {
    const isAuth = ctx.searchParams.get("auth") === "true";
    console.log(
      `[Auth Middleware] Checking auth for ${ctx.pathname}: ${isAuth}`,
    );
...
// Timed middleware
export const timedMiddleware: Middleware[] = [
  async (ctx, next) => {
    const start = performance.now();
    await next();
