...
> = {
  "server-components": {
    ...
    details:
      "Server Components allow you to write UI that can be rendered and optionally cached on the server. They run only on the server, reducing bundle size and improving performance.",
  },
  "server-actions": {
    name: "Server Actions",
    description: "Functions that run on the server",
    details:
      "Server Actions are async functions that run on the server. They can be called from Client Components and provide a simple way to handle form submissions and mutations.",
  },
  streaming: {
    name: "Streaming",
