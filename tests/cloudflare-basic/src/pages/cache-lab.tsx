...
import { CacheLabPulseLoader, getCacheLabProduct } from "../cache-lab-data.js";
import {
  CacheLabInvalidationPanel,
  CacheLabLivePulse,
  CacheLabProductGrid,
} from "../components/CacheLabClient.js";
...
export async function CacheLabPage(ctx: HandlerContext) {
  ...
  ctx.use(Meta)({
    name: "description",
    content:
      "An explorable Cloudflare cache lab for use cache, PPR, tags, and invalidation.",
  });
  ctx.use(Meta)(
    new Promise<{ title: string }>((resolve) =>
      setTimeout(
        () => resolve({ title: `Cache Lab - ${alpha.cacheToken}` }),
...
const CACHE_LAB_STYLES = `
  ...
  .cache-lab-console button { background: var(--lab-accent); border-radius: 999px; font-size: .82rem; font-weight: 800; padding: .65rem 1rem; }
  ...
  .dark .cache-lab { --lab-ink: #f5f0e5; --lab-muted: #aaa395; --lab-paper: #24221d; --lab-card: #1d1c18; --lab-line: #48443b; }
  .dark .cache-lab-hero-actions a { color: #171612; }
  .dark .cache-lab-hero-actions a + a { color: var(--lab-ink); }
