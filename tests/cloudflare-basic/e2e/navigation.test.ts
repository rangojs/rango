...
// Core client navigation (links, no-reload SPA transitions, browser
// back/forward, direct URL entry) on the Cloudflare (workerd) runtime. The
// assertions are deterministic content/visibility checks, so they hold
// identically in dev and the production build.
function describeNavigation(mode: "dev" | "build") {
  ...
  test.describe(`navigation (${label})`, () => {
    ...
    test("should render correct page on direct navigation", async ({
      page,
    }) => {
      using _ = expectNoPageError(page);
...
describeNavigation("dev");
describeNavigation("build");
