 ...
 * Breadcrumb content is deterministic (derived from the matched segments),
 * so the assertions hold identically in dev and the production build.
 */
function describeHandles(mode: "dev" | "build") {
  const label = mode === "build" ? "production" : "dev";
  test.describe(`handles (${label})`, () => {
    const f = useFixture({
      root: ".",
      mode,
    });
    ...
    test("should display nested breadcrumbs on about page", async ({
      page,
    }) => {
      using _ = expectNoPageError(page);
