...
// Back/forward and loading-state behavior around a JSON API (response) route.
// The loading-state probes hold the RSC partial deterministically (the held
// fetch keeps the navigation in "loading" until released), and the X-RSC-Reload
// short-circuit + bfcache pageshow reset are runtime behaviors that are
// identical in dev and the production build.
function describeApiRouteNavigation(mode: "dev" | "build") {
  const label = mode === "build" ? "production" : "dev";
  test.describe(`api route back navigation (${label})`, () => {
    const f = useFixture({
      root: ".",
      mode,
    });
    ...
    test("clicking api route link sets navigation to loading state", async ({
      ...
      // The navigation state is now "loading" because the RSC fetch is
      // in-flight. Wait for the spin-delay (400ms) to potentially show the
      // indicator.
...
describeApiRouteNavigation("dev");
describeApiRouteNavigation("build");
